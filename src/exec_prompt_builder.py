...
def _block_specific_policy_notes(block_spec: BlockSpec) -> List[str]:
    ...
    if block_spec.block_id == "rf_entry":
        ...
        notes.append("Absence of a RU public registration record MUST NOT automatically produce NO_GO. If no source-backed prohibition, failed/withdrawn/refused registration, or blocking legal/FTO fact is present, classify current_registration_status=NOT_REGISTERED_PUBLIC_RECORD, generic_entry_path=NOT_APPLICABLE_NO_REFERENCE_REGISTRATION, original_registration_path=POSSIBLE_BUT_UNPROVEN, and use HOLD/PARTIAL pending clinical-development, foreign-approval, regulatory-route, IP/FTO, and payer checks.")
